...
| Install locally | `code --install-extension claude-overwrite-features-0.6.45.vsix` |
...
| 2 | Strip attachments from slash commands + reset toggle | `webview/index.js` | Skips attachments when the message is a slash command and resets includeSelection to off after every send. v2.1.245: `await $.send(a,B,I1,{kind:"human"}),W([]),tk(Z,!0)` → `await $.send(a,Q1?[]:B,I1,{kind:"human"}),W([]),w(!1),tk(Z,!0)` |
