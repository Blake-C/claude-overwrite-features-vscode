...
| Install locally | `code --install-extension claude-overwrite-features-0.6.43.vsix` |
...
|---|---|---|
| 1 (includeSelection default) | `webview/index.js` | `"selectionLabel"` CSS class string nearby, or the main chat view component signature (v2.1.204: `function _Qe({session:`; component name changes each release). The three consecutive `useState` calls are `_=_e(!0),[C,x]=ie(!0),[y,w]=ie(!1)` (v2.1.204 used `ie`; v2.1.205 through v2.1.217 used `ne`; v2.1.221 onward is back to `ie`; the leading `useRef` alias was `Se` through v2.1.224, `we` in v2.1.226, `_e` in v2.1.228, `ge` in v2.1.229, `_e` again in v2.1.236, `me` in v2.1.239, and `_e` again in v2.1.241; the `useState` alias is `ne` again in v2.1.229, stays `ne` through v2.1.234, is `ie` from v2.1.235, is `te` in v2.1.239, and is `ie` again in v2.1.241) — flip the middle state pair's init from `!0` to `!1` (that pair was `[C,y]` through v2.1.226 and is `[C,x]` from v2.1.228 on) |
