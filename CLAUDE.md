...
| Install locally | `code --install-extension claude-overwrite-features-0.6.52.vsix` |
...
|---|---|---|---|
| 1 | Default include-file toggle to OFF | `webview/index.js` | Flips the includeSelection `useState` init from `!0` to `!1` in the main chat view. v2.1.260: `[w,N]=$1(!0)` → `[w,N]=$1(!1)` |
| 2 | Strip attachments from slash commands + reset toggle | `webview/index.js` | Skips attachments when the message is a slash command and resets includeSelection to off after every send. v2.1.260: `await $.send(a,B,l1,{kind:"human"}),W([]),Ph(Q,!0)` → `await $.send(a,O1?[]:B,l1,{kind:"human"}),W([]),N(!1),Ph(Q,!0)` |
| 3 | Confirm before compact | `webview/index.js` | Replaces the compact `onClick` with an inline `<dialog>` styled with VS Code CSS variables. v2.1.260: the `onCompact` prop is `Z`, so the dialog's confirm branch calls `Z()` |
