...
| Install locally | `code --install-extension claude-overwrite-features-0.7.2.vsix` |
...
| 2 | Strip attachments from slash commands | `webview/index.js` | Skips attachments when the message is a slash command. The reset-toggle half was dropped in v2.1.270 because the toggle setter no longer exists. v2.1.273: `await $.send(z1,W,w1,{kind:"human"}),B([]),hy(Q,!0)` → `await $.send(z1,r?[]:W,w1,{kind:"human"}),B([]),hy(Q,!0)` |
| 3 | Confirm before compact | `webview/index.js` | Replaces the compact `onClick` with an inline `<dialog>` styled with VS Code CSS variables. v2.1.273: the component is `f25` (was `b25`) and the `onCompact` prop is still `J`, so the dialog's confirm branch calls `J()` |
...
### Component map (v2.1.273)

The minifier generates these names and most of them change every release, so on a new version re-derive them from the anchors in "Finding patches after a version update" instead of trusting this table. Every name here was read out of the installed v2.1.273 bundle.

| Minified name | Role |
|---|---|
| `Nq0` | Main chat view. Owns the attached-files state `[W,B]` and the submit handler `V5`, so it contains the Feature 2 patch site. Signature: `function Nq0({session:$,context:J,onCreateNewSession:Z,onTeleportCheckout:Y})` |
| `f25` | Compact/context-usage button. Takes `{percentageUsed:$,onCompact:J,buttonClassName:Z}` and renders the circle percentage button. Feature 3 replaces its `onClick`. The wrapper `uz0` renders it and decides whether to show a button |
| `x25` | Selection chip in the footer, which replaced the include-selection toggle in v2.1.270. Takes `{currentSelection:$,onRemove:J}`, builds its label through `y25`, and its X button calls `onRemove`, which the footer sets to `session.dismissSelection()` |
...
The function that assembles the message content array injects the `<ide_opened_file>` and `<ide_selection>` tags, so search for `<ide_selection>`. That string occurs twice in v2.1.273 and the first hit is a parser, so take the second. The function that builds a compact message attaches `compactMetadata`, which occurs five times.

### Current release

**v2.1.273:** the minifier from v2.1.245 is still in use, and every change in this release is a rename. The component map above gives the renamed components. Features 1, 3, and 5 match as-is. Features 2 and 4 moved:

- Feature 1: `applySelectionUpdate` is unchanged and its parameter is still `$`, so the v0.7.0 strings match
- Feature 2: submit handler `V5` (via `z0`/useCallback), command text is the callback argument `z1`, isSlashCommand `r` (was `i`), selection flag `w1=!r` (was `r=!i`), attached files `W` with setter `B`, scroll function `hy` with ref `Q`. Site: `await $.send(z1,W,w1,{kind:"human"}),B([]),hy(Q,!0)`. `r` held the selection flag in v2.1.272 and holds isSlashCommand here, so both names the patch reads had to change. The handler's `catch(r)` shadows `r`, and `z1`, `r`, and `w1` are each reused elsewhere in the component, so match on the full send call
- Feature 3: the compact button component was renamed `b25`→`f25`, but its props are still `{percentageUsed:$,onCompact:J,buttonClassName:Z}`, so `onCompact` is still `J` and both halves of the `to` string match. Site: `click to compact\`,onClick:J,onMouseEnter:`
- Feature 4: the response local was renamed `K`→`G` and the stats helper `fh$`→`dh$`. Every other variable holds its v2.1.272 position (see the v2.1.273 map in the Feature 4 row below)
