...
| Install locally | `code --install-extension claude-overwrite-features-0.6.54.vsix` |
...
|---|---|---|---|
| 1 | Default include-file toggle to OFF | `webview/index.js` | Flips the includeSelection `useState` init from `!0` to `!1` in the main chat view. v2.1.266: `[L,b]=n(!0)` → `[L,b]=n(!1)` |
| 2 | Strip attachments from slash commands + reset toggle | `webview/index.js` | Skips attachments when the message is a slash command and resets includeSelection to off after every send. v2.1.266: `await $.send(W1,W,M0,{kind:"human"}),B([]),Ih(Q,!0)` → `await $.send(W1,W0?[]:W,M0,{kind:"human"}),B([]),b(!1),Ih(Q,!0)` |
| 3 | Confirm before compact | `webview/index.js` | Replaces the compact `onClick` with an inline `<dialog>` styled with VS Code CSS variables. v2.1.266: the `onCompact` prop is still `Z`, so the dialog's confirm branch calls `Z()` |
