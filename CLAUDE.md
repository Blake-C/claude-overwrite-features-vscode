...
| Install locally | `code --install-extension claude-overwrite-features-0.6.56.vsix` |
...
|---|---|---|---|
| 1 | Default include-file toggle to OFF | `webview/index.js` | **No patch site as of v2.1.270.** Claude Code removed the includeSelection toggle, so there is no `useState` init to flip. The v2.1.268 strings (`[L,E]=n(!0)` → `[L,E]=n(!1)`) are kept so the health check keeps reporting it. See the v2.1.270 notes below |
...
- Feature 1: **no patch site.** The main chat view is `pH0` (`useRef` alias `U1`, `useState` alias `l`) and it holds no includeSelection state at all. The footer `QH0` takes `currentSelection` and `onRemoveSelection` in place of the toggle, and renders the chip component `a75`, whose title reads `Showing Claude your current file selection (...)` and whose X button calls `session.dismissSelection()`. The submit handler computes the flag as `let n=!t` (`t` = isSlashCommand), so the current file goes out with every non-slash message. There is no `useState(!0)` left to flip. Do not invent a replacement: the three candidate rewrites are written up under "Feature 1 options for v2.1.270" in `CHANGELOG.md` and each one changes behavior, so the choice is the author's
