...
| Install locally | `code --install-extension claude-overwrite-features-0.6.51.vsix` |
...
|---|---|---|---|
| 1 | Default include-file toggle to OFF | `webview/index.js` | Flips the includeSelection `useState` init from `!0` to `!1` in the main chat view. v2.1.259: `[M,_]=Y1(!0)` → `[M,_]=Y1(!1)` |
| 2 | Strip attachments from slash commands + reset toggle | `webview/index.js` | Skips attachments when the message is a slash command and resets includeSelection to off after every send. v2.1.259: `await $.send(a,B,l1,{kind:"human"}),W([]),fy(Z,!0)` → `await $.send(a,R1?[]:B,l1,{kind:"human"}),W([]),_(!1),fy(Z,!0)` |
